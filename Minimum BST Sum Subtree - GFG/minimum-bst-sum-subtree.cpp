...
class Solution {
public:
    int solve(Node *root, int &minlen, int &sum, int target, bool &flag, int &maxi, int &mini)
{
    if (root == NULL)
        return 0;
    ...
    int leftlen = solve(root->left, minlen, leftsum, target, leftflag, leftmaxi, leftmini);
    int rightlen = solve(root->right, minlen, rightsum, target, rightflag, rightmaxi, rightmini);
    ...
    int minSubtreeSumBST(int target, Node *root) {
        int sum = 0, mini = 1e9, maxi = -1e9, minlen = 1e9;
        bool flag = true;
        solve(root, minlen, sum, target, flag, maxi, mini);
        
        if(minlen == 1e9) return -1;
